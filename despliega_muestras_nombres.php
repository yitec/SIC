...
$("#btn_guardar").live("click", function(event){
 if(confirm('¿Seguro que desea modificar las muestras?')){
...
<?
$result=mysql_query("select m.id_contrato,m.codigo,m.nombre_muestra,m.numero_muestra, c.consecutivo from tbl_muestras m,tbl_contratos c where c.id='".$_REQUEST['id']."' and m.id_contrato=c.consecutivo");
...
?>	
    ...
    <td><div align="center" class="Arial14Negro"><input type="text" name="txt_nombre_<?=$tot_muestras?>" value="<?=utf8_decode($row['nombre_muestra']);?>"></td>    
    </div></td>  
...
?>
    ...
    <input type="hidden" id="total_muestras" value="<?=$tot_muestras?>">
    <br />    
