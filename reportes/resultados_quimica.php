...
$result=mysql_query(" select a.codigo,a.id_contrato,r.id, r.fecha_ingreso,r.observaciones_analista, c.nombre from tbl_resultados r,tbl_categoriasanalisis c, tbl_analisis a where r.estado='"."0"."' and r.id_laboratorio='"."1"."' and r.id_analisis=a.id  and a.id_analisis=c.id");
...
?>
        <tr>
          ...
          <td><div align="left" class="Arial10Negro"><?=$row['observaciones_analista'];?></div></td>
          <td><div align="center" class="Arial14Negro"><a id="ver" href="aprueba_resultados.php?id=<?=$row['id'];?>&codigo=<?=$row['codigo'];?>&nombre=<?=utf8_encode($row['nombre']);?>&laboratorio=<? echo"1";?>&contrato=<?=$row['id_contrato'];?>"><img src="img/search.png" width="25" height="25" /></a></div></td>
        </tr>
        <?	
	}
