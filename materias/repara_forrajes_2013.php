...
$resu=mysql_query("select id, fecha_ingreso from bd_sic.tbl_resultados where fecha_ingreso BETWEEN DATE('2013-01-01 00:00:00') and Date('2013-12-22 00:00:00')");


while($ro=mysql_fetch_object($resu)){
	//echo "<br>****************************************************************";
	//echo "****************************************************************<br>";
	//echo "Id Resultado=".$ro->id."<br>";
	//echo "Fecha Ingreso=".$ro->fecha_ingreso."<br>";
	
	
	actualiza_materias($ro->id);
}
...
function actualiza_materias($id_resultado)
{
	...
	$result=mysql_query("select * from bd_sic.tbl_resultados where id='".$id_resultado."' ")or throw_ex(mysql_error());
	$r1=mysql_fetch_object($result);
	
	$result=mysql_query("select * from bd_sic.tbl_analisis where id='".$r1->id_analisis."' ");
	...
	$result=mysql_query("select c.nombre from bd_sic.tbl_categoriasanalisis c,bd_sic.tbl_resultados r,bd_sic.tbl_analisis a where r.id='".$id_resultado."' and a.id= r.id_analisis and c.id=a.id_analisis ")or throw_ex(mysql_error());	
	...
	//echo $nombre."<br>";
//if ($row->nombre=="Cenizas"||$row->nombre==utf8_decode('Proteína Cruda')||$row->nombre=="Fibra Cruda"||$row->nombre==utf8_decode("Extracto etéreo")||$row->nombre==utf8_decode("Humedad 135° C")){
//echo "entro en ".$nombre."<br>";
 //mysql_free_result($result);



 if($r3->id_categoria==5){ 	
	$res=mysql_query("select tipo_alimento from bd_sic.tbl_infmuestras where cons_contrato='".$r3->id_contrato."'")or throw_ex(mysql_error());	
	$ros=mysql_fetch_object($res);
	echo "contrato= ".$r3->id_contrato." Tipo muestra=".$ros->tipo_alimento." id_muestra= ".$r3->id;
	echo "<br>****************************************************************<br>";
	...
	*/
	$rec=mysql_query("select id from bd_materiasprimas.tbl_forrajes where id_muestra='".$r3->id."' ");
...
if ($encontrado==0&&$r3->id_categoria==5){

	echo $r3->id_contrato."  Entro en clasificacion de datos<br>";
	
	$result=mysql_query("select * from bd_sic.tbl_contratos where consecutivo='".$r1->consecutivo_contrato."' ");
	$r4=mysql_fetch_object($result);
	
	$result=mysql_query("select * from bd_sic.tbl_infmuestras where cons_contrato='".$r1->consecutivo_contrato."' ");
	$r5=mysql_fetch_object($result);
	
	$result=mysql_query("select * from bd_sic.tbl_infforrajes where cons_contrato='".$r1->consecutivo_contrato."' ");
	$r6=mysql_fetch_object($result);
	
	$result=mysql_query("select * from bd_sic.tbl_infoficiales where cons_contrato='".$r1->consecutivo_contrato."' ");
	$r7=mysql_fetch_object($result);
	
	$result=mysql_query("select * from bd_sic.tbl_clientes where id='".$r4->id_cliente."' ");
	...
	echo $nitro=busca_nitrogeno($r6->nitrogeno);			
		...
		/*
	 $year=busca_year($r5->fecha_muestra);	
	 $year=29;
...
}//end if si tiene los analisis de proximal	
*/	
	}//end try
	catch (Exception $e)
	{
...
function actualiza_resultados($id_resultado,$id_muestra,$id_categoria){
		...
		switch ($nombre){
			...
			case "Cenizas":
				...
				$result=mysql_query("Select resultado,base_fresca from bd_sic.tbl_resultados where id='".$id_resultado."'")or throw_ex(mysql_error());
				$r9=mysql_fetch_object($result);
				if ($r9->base_fresca<>""){							
					mysql_query("update ".$tabla." set cenizas='".$r9->base_fresca."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	

				}else{								
					mysql_query("update ".$tabla." set cenizas='".$r9->resultado."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}
			...
			case "Proteína Cruda":						
				...
				$r9=mysql_fetch_object($result);
				if ($r9->base_fresca<>""){							
					mysql_query("update ".$tabla." set proteina_cruda='".$r9->base_fresca."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}else{								
					mysql_query("update ".$tabla." set proteina_cruda='".$r9->resultado."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}
			...
			case "Fibra Cruda":						
				...
				$r9=mysql_fetch_object($result);
				if ($r9->base_fresca<>""){							
					mysql_query("update ".$tabla." set fibra_cruda='".$r9->base_fresca."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}else{								
					mysql_query("update ".$tabla." set fibra_cruda='".$r9->resultado."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}
			...
			case "Extracto etéreo":						
				...
				$r9=mysql_fetch_object($result);
				if ($r9->base_fresca<>""){							
					mysql_query("update ".$tabla." set extracto_etereo='".$r9->base_fresca."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}else{								
					mysql_query("update ".$tabla." set extracto_etereo='".$r9->resultado."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}
			...
			case "Humedad 135° C":		
				...
				$r9=mysql_fetch_object($result);
				if ($r9->base_fresca<>""){							
					mysql_query("update ".$tabla." set humedad_135='".$r9->base_fresca."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}else{								
					mysql_query("update ".$tabla." set humedad_135='".$r9->resultado."' where id_muestra='".$id_muestra."'")or throw_ex(mysql_error());	
				}
