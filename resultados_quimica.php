...
<div class="cen_sup_g"><div  class="Arial14blanco" align="left" style="float:left; margin-top:18px;">Resultados</div><div align="right"></div> </div>
...
<div id="mainBlancoFondo"  >
	...
	<div align="center" class="Arial18Morado" style="margin-bottom:10px; margin-top:10px;">Resultados Qu&iacute;mica</div>
    ...
    <table cellpadding="0" cellspacing="0" border="0">
      ...
      <table width="747" height="18" border="1"   cellpadding="0" cellspacing="0" bordercolor="#a6c9e2">
        <tr>
          ...
          <td><div align="center" class="azulColumn">Codigo Analisis</div></td>          
...
$result=mysql_query(" select a.id_contrato,a.id as id_analisis, a.codigo,a.id_contrato,r.id, r.fecha_ingreso,r.observaciones_analista, c.nombre from 
  tbl_resultados r join tbl_analisis a
  on r.id_analisis=a.id
  join tbl_categoriasanalisis c
  on a.id_analisis=c.id
  where r.estado='"."0"."' and r.id_laboratorio='"."1"."'");
...
?>
        <tr>
          ...
          <td>
          <?if ($row['nombre']=='NIR'){?>
            <div align="center" class="Arial14Negro"><a id="ver" href="aprueba_resultados_nir.php?id=<?=$row['id'];?>&codigo=<?=$row['codigo'];?>&nombre=<?=utf8_encode($row['nombre']);?>&laboratorio=<? echo"1";?>&contrato=<?=$row['id_contrato'];?>&id_analisis=<?=$row['id_analisis'];?>&contrato=<?=$row['id_contrato'];?>"><img src="img/search.png" width="25" height="25" /></a>
            </div>
          <?}else{?>
            <div align="center" class="Arial14Negro"><a id="ver" href="aprueba_resultados.php?id=<?=$row['id'];?>&codigo=<?=$row['codigo'];?>&nombre=<?=utf8_encode($row['nombre']);?>&laboratorio=<? echo"1";?>&contrato=<?=$row['id_contrato'];?>&id_analisis=<?=$row['id_analisis'];?>&contrato=<?=$row['id_contrato'];?>"><img src="img/search.png" width="25" height="25" /></a>
            </div>
          <?}?>
          </td>
        </tr>
