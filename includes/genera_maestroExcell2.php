...
$consulta = "SELECT nombre_categoria,codigo,fecha_creacion, ultima_revision,prefijo,id_usuario, responsable, nombre_subcat, nombre_archivo, version, url_archivo, url_online FROM vista_maestro WHERE estado = 1";
...
while ($row=mysql_fetch_object($result)){
      ...
      if ($nombre_categoria!=$row->nombre_categoria){
      ...
      if ($pestanas==0){
            ...
            ->setCellValue('G'.$registros, 'Copias controladas')
      ...
      $pestanas++;
            ...
            $registros++;
            $objPHPExcel->setActiveSheetIndex($pestanas-1)
            ->setCellValue('A'.$registros, $row->nombre_categoria)
            ->setCellValue('B'.$registros, $row->prefijo."-".$row->codigo)
