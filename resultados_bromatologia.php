...
$result=mysql_query(" select a.id as id_analisis, a.codigo,a.id_contrato,r.id, r.fecha_ingreso,r.observaciones_analista, c.nombre from tbl_resultados r,tbl_categoriasanalisis c, tbl_analisis a where r.estado='"."0"."' and r.id_laboratorio='"."3"."' and r.id_analisis=a.id  and a.id_analisis=c.id");
...
?>
        <tr>
          ...
          <td><div align="center" class="Arial14Negro"><a id="ver" href="aprueba_resultados.php?id=<?=$row['id'];?>&codigo=<?=$row['codigo'];?>&nombre=<?=utf8_encode($row['nombre']);?>&laboratorio=<? echo"3";?>&contrato=<?=$row['id_contrato'];?>&id_analisis=<?=$row['id_analisis'];?>"><img src="img/search.png" width="25" height="25" /></a></div></td>
        </tr>
        <?	
	}
