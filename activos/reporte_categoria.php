...
<div align="center" class="Arial18Azul" style="margin-bottom:10px; margin-top:10px;"> Reporte:  Total de activos por Categoria</div>    
	</br>
...
<form action="ficheroExcel.php" method="post" target="_blank" id="FormularioExportacion">
	...
	<input type="hidden" id="Nombre" name="Nombre" value="ReporteCategoria" />          
	...
	<?php
		...
		if($query_result){
				...
				while($row = mysql_fetch_array($query_result))
				{
					...
					where ActCat.id_categoria =" . $row[0]  . "  order by 1";
		
					$query_result_datos=mysql_query($queryE,$_SESSION['conectact']);
					...
					if( mysql_num_rows($query_result_datos) > 0 ){
						while($rowE = mysql_fetch_array($query_result_datos))
						{
							if($rowE[11]==1){
								$oaf = "Si";
							}else{
